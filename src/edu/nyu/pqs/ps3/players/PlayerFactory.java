 ...
 * PlayerFactory auto-generates the player id and keeps track of maximum players that
 * can be allowed in a game. It also generates random color if none provided.
 */
public class PlayerFactory {
     ...
     * @throws IllegalStateException if max number of players already initialized
     */
    public static Player getPlayer(PlayerType type, String name, Color color, AIParameters parameters) {
        if (currentPlayerCount >= maxNoOfPlayers)
            throw new IllegalStateException("Max number of players initialized");
        if (type == null)
    ...
    /**
     * @param count Used to define the maximum number of players in a game
     */
    public static void setMaxPlayers(int count){
        maxNoOfPlayers = count;
    }

    /**
     * @return Maximum number of players that can play the game
     */
