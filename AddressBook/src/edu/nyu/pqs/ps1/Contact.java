...
/**
 * The contact class stores individual entries of the address book. It consists
 * of the details of a contact which are Name (mandatory), phone-number,
 * address, email-address and a note. The class is constructed using the Builder
 * pattern and the object created is immutable. A class is instantiated with a
 * name and you can add additional details with the use of functions like
 * .email(), .phone() etc.
 *
 * The class overrides the equals() and hashCode() functions to use the class
 * with generics. toString() is implemented to make the contact information
 * readable. The class also creates a COMPARATOR to make it usable by SortedSets
 * and similar Collections.
 *
 * The modify method constructs and returns a new Object with one field changed
...
public class Contact implements Comparable<Contact> {
    ...
    /**
     * Since the Contact is immutable we create a new Object and return it with the
     * changed field. This method is package-private and can only be called by using
     * the modifyContact() method in the AddressBook class
     *
    ...
    /**
     * Returns the comparison between two contacts and compares all the field
     * values. For equivalency of contacts '+()-' characters are ignored from
     * phone-numbers.
     *
    ...
    public boolean equals(Object o) {
        ...
        if (contact.name.equals(name)) {
            if (contact.getPhone().replaceAll("[^\\d]", "")
                .equals(this.getPhone().replaceAll("[^\\d]", ""))) {
    ...
    /**
     * Creates a string of contact information based on all available(not null)
     * fields. It follows lazy initialization and created the string the first time
     * the toString() is called since the class is immutable
     *
    ...
    /**
     * Initialize a comparator to make the class be used efficiently with generics
     * and Collections
     */
    ...
    /**
     * Builder subclass to implement the Builder Pattern, wherein the name is passed
     * to the Builder constructor and further parameters can be added using the
     * email(), address() etc. functions, where the null or blank values are omitted
     * from the object. All validations are performed inside the builder functions
     * and throw IllegalArgumentException with different messages according to the
     * validation and the field
     */
    public static class Builder {
        ...
        public Builder phoneNumber(String val) {
            /*
             * Use Regular expression to evaluate the phone number (\+[0-9][0-9]?)? matches
             * international codes +1 or +91 \d{10} matches 1234567890 (?:\d{3}-){2}\d{4}
             * matches 123-456-7890 \(\d{3}\)\d{3}-?\d{4} matches (123)456-7890 or
             * (123)4567890
             */
            if (val != null && !val.equals("")) {
        ...
        public Builder email(String val) {
            /*
             * Use Regular expression to evaluate the email [\w-_.+]*[\w-_.]@1 or more
             * characters with -_and . followed by @ ([\w]+\.)+ matches 1 or more domains
             * like cs.nyu. or pqs.cs.nyu. [\w]+[\w] ends with an org like edu or com
             */
            if (val != null && !val.equals("")) {
        ...
        /**
         * Build and return the Contact by passing in the builder object created from
         * the methods above
         *
